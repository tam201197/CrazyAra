...
public:
    ...
    void revert_virtual_loss_and_update(ChildIdx childIdx, float value, const SearchSettings* searchSettings, bool solveForTerminal, bool isMaxOperator)
    {
        ...
        if (searchSettings->virtualLoss != 1) {
            ...
            d->childNumberVirtualVisits[childIdx] -= size_t(searchSettings->virtualLoss) - 1;
    ...
    for (auto it = trajectory.rbegin(); it != trajectory.rend(); ++it) { 
        ...
        case BACKUP_MAX:
            if (it->node->get_child_node(it->childIdx) == nullptr) {
                ...
                uint32_t child_real_visits = 0;
                it->node->get_child_node(it->childIdx)->lock();
                child_real_visits = it->node->get_child_node(it->childIdx)->get_real_visits();
                it->node->get_child_node(it->childIdx)->unlock();
                if (child_real_visits >= searchSettings->switchingMaxOperatorAtNode) {
